...
$(function() {
	...
	$(window).on("resize", debounce( function() {
		...
		var result = calculateAspectRatioFit($video.width(), $video.height(), $banner.width(), $banner.height());
...
function replaceMenuBtn($menuBtn, $mobileHeader) {
	$menuBtn.detach();
	...
	$closeBtn.appendTo($mobileHeader);
}


function redoMenuBtn($menuBtn, $mobileHeader) {
	$(".goodpopup-close").remove();

	$menuBtn.appendTo($mobileHeader);
}
...
function callPopupMenu() {
	...
	var $menuBtn = $(".c-mobile-header__menu-btn");
	var $mobileHeader = $(".c-mobile-header");
	popup.setOptions({
		isOuterClickClosing: true,
		callbackBeforeOpen: function() {
			setTimeout(function() {
				...
				replaceMenuBtn($menuBtn, $mobileHeader);
			});
		},
		callbackBeforeClose: function() {
			redoMenuBtn($menuBtn, $mobileHeader);
		}
	});
	popup.open();
}
...
$(".c-mobile-header__menu-btn").on("click", callPopupMenu);
...
// $(".footer-rules-a").on("click", callPopupRules);
	...
	var $input = $(".c-contact-form__email");

	$('.c-contact-form_lang_ru').submit(function() {
		var $this = $(this);
		...
		$this.ajaxSubmit({
			...
			success: function(data) {
				...
				if (outputString.indexOf("Ваше сообщение отправлено.") !== -1) {
					$input.attr("placeholder", "Сообщение отправлено!");
				} else {
					$input.attr("placeholder", "Ошибка, попробуйте ещё.");
				}
			},
			error: function() {
				$this[0].reset();
	...
	$('.c-contact-form_lang_en').submit(function() {
		var $this = $(this);
		...
		$this.ajaxSubmit({
			...
			success: function(data) {
				...
				$input.html(outputString);
				if (outputString.indexOf("Ваше сообщение отправлено.") !== -1) {
					$input.attr("placeholder", "Message sent!");
				} else {
	...
	//animations
	// for (var i = 0; i < portfolioBrands.length; i++) {
	// 	(function(i) {
	// 		$(".c-card_bg_" + portfolioBrands[i]).hover( 
	// 			//hover in
	// 			function() {
	// 				TweenMax.to($(this), 1, {backgroundImage:'linear-gradient(rgba(0, 0, 0, 0.75), rgba(0, 0, 0, 0.75)), url("assets/img/avatars/portfolio-' + portfolioBrands[i] + '.jpg")'});
	// 			},
	// 			// hover out
	// 			function() {
	// 				TweenMax.to($(this), 1, {backgroundImage:'url("assets/img/avatars/portfolio-' + portfolioBrands[i] + '.jpg")'});
	// 			}
	// 		);
	// 	})(i);
	// }
